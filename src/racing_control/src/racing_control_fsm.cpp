...
enum class State {
    IDLE,               // 空闲，等待启动
    LINE_FOLLOW,        // 巡线中
    QRCODE_SCAN,        // 扫描二维码，获取 CW(3)/CCW(4)
    REVERSE_TURN,       // 原地掉头转约90度，IMU判断角度
    TRACK_CHANNEL,      // 追踪"通"字（入口）
    YELLOW_ENTER,       // 根据CW/CCW转向进入黄色通道
    YELLOW_FOLLOW,      // 黄色通道巡线
    GET_PICTURE,        // 拍照，发送拍照指令
    TRACK_CHANNEL_END,  // 再次追踪"通"字（出口，到达结束阈值）
    EXIT_CHANNEL,       // 出通道后向前行驶，优先追P点否则巡线
    P_TRACK,            // 追踪P点
    P_STOP,             // 停车
    FINISH              // 完成
};
...
public:
        ...
        : Node(node_name, options)
    {
        ...
        declare_parameter("channel_enter_y", 400);  // 入口（进入黄色通道）
        declare_parameter("channel_end_y",   300);  // 出口（离开黄色通道后追踪）
        // REVERSE_TURN 目标偏转角度（弧度，约90度）
        declare_parameter("reverse_turn_angle", 1.57);
        // REVERSE_TURN 速度
        declare_parameter("reverse_linear_x",  -0.3); // 倒车
        declare_parameter("reverse_angular_z",  0.5); // 转向
        // YELLOW_ENTER 转向速度
        declare_parameter("yellow_enter_angular_z", 0.5);
        declare_parameter<double>("yellow_enter_x", yellow_enter_x_);
        // YELLOW_FOLLOW
        declare_parameter("yellow_x",  0.5);
        declare_parameter("yellow_kp", 0.005);
        declare_parameter("yellow_kd", 0.001);
        // GET_PICTURE 触发一次后自动结束，无需等待
        // EXIT_CHANNEL
        ...
        get_parameter("yellow_enter_x", yellow_enter_x_);
        get_parameter("yellow_x",  yellow_x_);
        get_parameter("yellow_kp", yellow_kp_);
        get_parameter("yellow_kd", yellow_kd_);
        ...
        // sign_switch（CW(3)/CCW(4)方向指令以及状态控制）
        ...
        // ==== 发布 ====
        racing_pub_  = create_publisher<geometry_msgs::msg::Twist>("/cmd_vel", qos_be);
        ...
        yellow_dir_pub_ = create_publisher<std_msgs::msg::Int32>("/yellow_direction", 10);
...
private:
    ...
    rclcpp::Publisher<std_msgs::msg::Int32>::SharedPtr                yellow_dir_pub_;
    ...
    void imu_callback(const sensor_msgs::msg::Imu::SharedPtr msg) {
        ...
        last_imu_time_ = now;
        // angular_velocity.z 为角速度，取绝对值累加
    ...
    void sign_callback(const origincar_msg::msg::Sign::SharedPtr msg) {
        ...
        if (val == 10) {
         // 测试用，直接切换到YELLOW_FOLLOW，顺时针
         yellow_dir_ = 3;//顺时针
         RCLCPP_INFO(get_logger(), "sign=10 test: -> YELLOW_FOLLOW");
         switch_state(State::YELLOW_FOLLOW);
        }
        if (val == 11) {
         // 测试用，直接切换到YELLOW_FOLLOW，逆时针
         yellow_dir_ = 4;//逆时针
         RCLCPP_INFO(get_logger(), "sign=11 test: -> YELLOW_FOLLOW");
         switch_state(State::YELLOW_FOLLOW);
        }
        if (val == 12) {
         // 测试用，直接切换到顺时针掉头90度方向的REVERSE_TURN
         yellow_dir_ = 3;//顺时针
         std_msgs::msg::Int32 dir_msg;
         dir_msg.data = yellow_dir_;
         yellow_dir_pub_->publish(dir_msg);
         RCLCPP_INFO(get_logger(), "sign=12 test: -> REVERSE_TURN");
         switch_state(State::REVERSE_TURN);
        }
        if (val == 13) {
         // 测试用，直接切换到逆时针掉头90度方向的REVERSE_TURN
         yellow_dir_ = 4;//逆时针
         std_msgs::msg::Int32 dir_msg;
         dir_msg.data = yellow_dir_;
         yellow_dir_pub_->publish(dir_msg);
         RCLCPP_INFO(get_logger(), "sign=13 test: -> REVERSE_TURN");
         switch_state(State::REVERSE_TURN);
        }
        if (val == 0) {
         // 测试用，直接切换到停止状态
         ...
         dir_msg.data = 0;
         yellow_dir_pub_->publish(dir_msg);
         RCLCPP_INFO(get_logger(), "sign=0 test: -> IDLE");
         switch_state(State::IDLE);
        }
        if (val == 14) {
         // 测试用，直接切换到
         yellow_dir_ = 3;//顺时针
         RCLCPP_INFO(get_logger(), "sign=14 test: -> YELLOW_ENTER");
         switch_state(State::YELLOW_ENTER);
        }
        if (val == 15) {
         // 测试用，直接切换到逆时针掉头90度方向的REVERSE_TURN
         yellow_dir_ = 4;//逆时针
         RCLCPP_INFO(get_logger(), "sign=15 test: -> YELLOW_ENTER");
         switch_state(State::YELLOW_ENTER);
        }
    }
    ...
    void switch_state(State next) {
        ...
        if (next == State::YELLOW_FOLLOW || next == State::YELLOW_ENTER) {
            std_msgs::msg::Int32 dir_msg;
            dir_msg.data = yellow_dir_;
            yellow_dir_pub_->publish(dir_msg);
}
    ...
    struct FilteredTargets {
        ...
        std::vector<ai_msgs::msg::Target> tong;     // "通"字
        std::vector<ai_msgs::msg::Target> dao;      // "道"字
    };
        ...
        const ai_msgs::msg::PerceptionTargets::SharedPtr &target_msg)
    {
        ...
        for (const auto &t : target_msg->targets) {
            ...
            else if (t.type == "tong") ft.tong.push_back(t);
            else if (t.type == "dao")  ft.dao.push_back(t);
        }
        return ft;
    }
    ...
    // 取"通道"目标：优先取"通"字，找不到再用"道"字
    static const ai_msgs::msg::Target* channel_target(const FilteredTargets &ft)
    {
        const auto *tong = max_area_target(ft.tong);
        if (tong) return tong;
        return max_area_target(ft.dao);
    }
        ...
        const ai_msgs::msg::PerceptionTargets::SharedPtr &point_msg)
    {
        ...
        // 避障：预判二维码在右侧时，next_target_x=640
    ...
    // is_end=false 入口（底边到达阈值channel_enter_y_）
    // is_end=true  出口（底边到达阈值channel_end_y_）
    void state_track_channel(const FilteredTargets &ft, bool is_end) {
        ...
        // 避障：预判通道字在前方，next_target_x=320
        if (do_avoid_zt(ft, avoid_x_, 320.0)) return;

        const auto *ch = channel_target(ft);
        if (!ch) {
            // 找不到通道文字，缓慢前进
    ...
    void state_yellow_enter(double yellow_error) {
        ...
        publish_twist(yellow_enter_x_, angular_z);
        RCLCPP_INFO(get_logger(),
            "[YELLOW_ENTER] dir=%d angular_z=%.3f", yellow_dir_, angular_z);
    }

    // YELLOW_FOLLOW: 黄色通道巡线，内部含避障
    void state_yellow_follow(const FilteredTargets &ft, double yellow_error) {
        ...
        // TODO: GET_PICTURE触发条件，目前依赖前方图文标识牌
        // 当YOLO检测到图文标识牌时触发，目前用flag位
        ...
        double d_error = error_filtered - last_yellow_error_;  // 误差变化率
        double angular_z = yellow_kp_ * error_filtered + yellow_kd_ * d_error;
    ...
    // GET_PICTURE: 拍照后立即转YELLOW_FOLLOW
