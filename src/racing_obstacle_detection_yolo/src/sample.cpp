...
ObstacleDetectionNode::ObstacleDetectionNode(const std::string& node_name,
  ...
  sub_qos.reliability(RMW_QOS_POLICY_RELIABILITY_BEST_EFFORT);  // 订阅用best_effort

  rclcpp::QoS pub_qos(1);
  pub_qos.reliability(RMW_QOS_POLICY_RELIABILITY_RELIABLE);  // 发布用reliable
  ...
  msg_publisher_ = this->create_publisher<ai_msgs::msg::PerceptionTargets>(
      "/racing_obstacle_detection", pub_qos);
}
